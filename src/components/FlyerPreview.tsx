...
export const FlyerPreview = forwardRef<ViewShot, FlyerPreviewProps>(({ flyer }, ref) => {
    ...
    return (
        <ViewShot ref={ref} options={{ format: 'png', quality: 0.9 }}>
            <View className="w-full bg-white relative overflow-hidden shadow-sm rounded-xl mb-8">
                {/* Header */}
                {theme.bannerUrl ? (
                ...
                {/* Content */}
                <View className={`p-5 ${theme.bgClass}`}>
                    {!theme.bannerUrl && (
                    ...
                    <View key={`grid-${flyer.products.length}`} className="flex-row flex-wrap justify-between">
                        {flyer.products.map((product: Product) => {
                            ...
                            return (
                                <View key={product.id} collapsable={false} className="w-[48%] bg-white rounded-2xl p-3 shadow-sm border border-gray-100 relative mb-4">
                                    {discount > 0 && (
                                    ...
                                    <View className="h-40 rounded-xl bg-gray-50 mb-3 overflow-hidden items-center justify-center p-2">
                                        {product.imageUrl ? (
                                    ...
                                    <View collapsable={false} className="min-h-[60px]">
